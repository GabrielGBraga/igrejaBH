...
export default function RedeRelacionamentos() {
  ...
  const [activeTab, setActiveTab] = useState<"grafo" | "membros" | "setores">(
    "grafo"
  )
  ...
  const [activeConnectionType, setActiveConnectionType] = useState<
    "discipler" | "fellowship" | "group_leader"
  >("discipler")

  // Zoom & Pan
  const [isolateSelectedConnections, setIsolateSelectedConnections] =
    useState(false)
  ...
  const [newConnectionType, setNewConnectionType] = useState<
    "is_discipler" | "is_disciple" | "fellowship" | ""
  >("")
  ...
  const [selectedHomeGroupToEdit, setSelectedHomeGroupToEdit] =
    useState<HomeGroup | null>(null)
  const [editHomeGroupMeetingDay, setEditHomeGroupMeetingDay] =
    useState<number>(1)
  const [editHomeGroupStartTime, setEditHomeGroupStartTime] =
    useState<string>("20:00")
  const [editHomeGroupLocationText, setEditHomeGroupLocationText] =
    useState<string>("")
  const [editHomeGroupLeader1Id, setEditHomeGroupLeader1Id] =
    useState<string>("")
  const [editHomeGroupLeader2Id, setEditHomeGroupLeader2Id] = useState<
    string | null
  >(null)
  ...
  const [editHomeGroupSectorId, setEditHomeGroupSectorId] = useState<
    string | null
  >(null)
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (fellowshipsError) {
        console.warn(
          "Could not fetch fellowships (check RLS):",
          fellowshipsError
        )
      }
  ...
  const handleGeocodeEditAddress = async () => {
    ...
    try {
      ...
      if (!found) {
        toast.error(
          "Não foi possível encontrar a localização. Por favor, ajuste o endereço."
        )
      }
    } catch (error) {
  ...
  const handleConnect = async () => {
    ...
    try {
      if (newConnectionType === "is_disciple") {
        ...
        const alreadyExists = fellowships.some(
          (f) =>
            (f.member_a_id === selectedNodeId &&
              f.member_b_id === newConnectionTargetId) ||
            (f.member_a_id === newConnectionTargetId &&
              f.member_b_id === selectedNodeId)
        )
        ...
        const { error } = await supabase.from("fellowships").insert({
    ...
    try {
      if (typeName === "Discipulador") {
        ...
        const { error } = await supabase
          ...
          .delete()
          .or(
            `and(member_a_id.eq.${selectedNodeId},member_b_id.eq.${targetId}),and(member_a_id.eq.${targetId},member_b_id.eq.${selectedNodeId})`
          )
        if (error) throw error
  ...
  const getGroupLeadersName = useCallback(
    (hg: HomeGroup) => {
      const leader1 = hg.leader_1_id
        ? profileMap.get(hg.leader_1_id)?.full_name
        : null
      const leader2 = hg.leader_2_id
        ? profileMap.get(hg.leader_2_id)?.full_name
        : null
  ...
  const sectorsList = useMemo(() => {
    ...
    return Array.from(new Set([...dbSectors, ...parsedSectors]))
      .filter(Boolean)
      .sort()
  }, [sectors, homeGroups])
  ...
  const rawGraph = useMemo(() => {
    ...
    const uniqueDisciplers = new Set(
      profiles.map((p) => p.discipler_id).filter((id): id is string => !!id)
    )
    ...
    profiles.forEach((p) => {
      ...
      while (currentId) {
        ...
        if (
          !currentProfile.discipler_id ||
          !profileMap.has(currentProfile.discipler_id)
        ) {
          let baseLevel = 1
          const isLeader = leaderIds.has(currentProfile.id)
          const isDiscipler = uniqueDisciplers.has(currentProfile.id)
          const isChild =
            !!(currentProfile.father_id || currentProfile.mother_id) &&
            !currentProfile.baptism_date
    ...
    const nodes: GraphNode[] = profiles.map((p, idx) => {
      ...
      const hgSector =
        hg && hg.sector_id && sectorMap.has(hg.sector_id)
          ? sectorMap.get(hg.sector_id)!.name
          : hg
            ? getSectorFromLocation(hg.location_text)
            : "Sem Setor"
      ...
      const genderMapped =
        p.gender === "F" || p.gender === "feminino"
          ? "feminino"
          : p.gender === "M" || p.gender === "masculino"
            ? "masculino"
            : p.gender
    ...
    profiles.forEach((p) => {
      if (p.home_group_id) {
        ...
        if (hg) {
          const leaders = [hg.leader_1_id, hg.leader_2_id].filter(
            (id): id is string => !!id && id !== p.id
          )
  ...
  const filteredData = useMemo(() => {
    ...
    const filteredLinks = links.filter((link) => {
      const isTypeMatch =
        link.type === activeConnectionType || link.type === "marriage"
  ...
  const stats = useMemo(() => {
    ...
    const totalGcs = new Set(
      filteredData.nodes.map((n) => n.homeGroupId).filter(Boolean)
    ).size
    const totalLeaders = filteredData.nodes.filter((n) =>
      leaderIds.has(n.id)
    ).length

    const disciplesWithDiscipler = filteredData.nodes.filter(
      (n) => n.disciplerId
    )
    const uniqueDisciplers = new Set(
      disciplesWithDiscipler.map((n) => n.disciplerId)
    )
    const avgDisciplesPerDiscipler =
      uniqueDisciplers.size > 0
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          <Skeleton className="col-span-1 h-24" />
          <Skeleton className="col-span-1 h-24" />
          <Skeleton className="col-span-1 h-24" />
          <Skeleton className="col-span-1 h-24" />
        </div>
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-4">
          ...
          <Skeleton className="h-[500px] rounded-2xl lg:col-span-1" />
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="animate-in space-y-6 duration-500 fade-in slide-in-from-bottom-4">
      {/* Header */}
      <header className="flex flex-col justify-between gap-4 border-b border-border/50 pb-6 md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground md:text-base">
            Visualize e filtre as juntas e ligamentos (Atos 2) do Corpo de
      ...
      {/* Tabs */}
      <div className="mb-2 flex gap-2 border-b border-border/50">
        <button
          type="button"
      ...
      {activeTab === "grafo" ? (
        <>
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            <Card className="border-border/50 bg-card/20 shadow-sm backdrop-blur-sm">
            ...
            <Card className="border-border/50 bg-card/20 shadow-sm backdrop-blur-sm">
              <CardContent className="flex items-center gap-3 p-4">
                ...
                <div>
                  <p className="text-xs font-medium tracking-wider text-muted-foreground uppercase">
                    Média p/ Discipulador
                  </p>
                  <h3 className="text-xl font-bold text-foreground">
                    {stats.avgDisciplesPerDiscipler}
                  </h3>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="grid grid-cols-1 items-stretch gap-6 lg:grid-cols-4">
            ...
            <div className="flex flex-col space-y-6 lg:col-span-1">
              {/* Filters Card */}
              <Card className="flex flex-col border-border/50 bg-card/20 shadow-sm backdrop-blur-sm">
                ...
                <CardContent className="space-y-4 p-4 pt-2 text-sm">
                  {/* Name Search */}
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem
                          value="create_new"
                          className="mt-1.5 cursor-pointer border-t border-border/50 pt-2 font-semibold text-primary hover:bg-primary/5 focus:bg-primary/5"
                        >
                  ...
                  <div className="space-y-2">
                    <Label className="text-xs font-semibold text-muted-foreground">
                      Tipo de Conexão (Ver uma por vez)
                    </Label>
                    <div className="grid grid-cols-1 gap-1 rounded-xl border border-border/30 bg-zinc-100/50 p-1 dark:bg-zinc-800/30">
                      <button
                        type="button"
                        onClick={() => setActiveConnectionType("discipler")}
                        className={`flex items-center gap-2 rounded-lg px-3 py-1.5 text-left text-xs font-medium transition-all ${
                          activeConnectionType === "discipler"
                      ...
                      >
                        <span
                          className={`h-2 w-2 rounded-full ${activeConnectionType === "discipler" ? "bg-white" : "bg-indigo-500"}`}
                        ></span>
                        Discipulado
                      </button>
                      <button
                      ...
                      >
                        <span
                          className={`h-2 w-2 rounded-full ${activeConnectionType === "fellowship" ? "bg-white" : "bg-teal-500"}`}
                        ></span>
                      ...
                      >
                        <span
                          className={`h-2 w-2 rounded-full ${activeConnectionType === "group_leader" ? "bg-white" : "bg-amber-500"}`}
                        ></span>
                        Liderança do GC
                      </button>
                    </div>
                    ...
                    <div className="mt-2 flex items-center space-x-2 border-t border-border/30 pt-2">
                      ...
                      <Label
                        ...
                        className={`flex cursor-pointer items-center gap-1.5 text-xs font-semibold ${!selectedNodeId ? "cursor-not-allowed opacity-50" : "text-foreground"}`}
                      >
              ...
              {/* Details Card */}
              {selectedNode ? (
                <Card className="animate-in border-border/50 bg-card/20 shadow-sm backdrop-blur-sm duration-200 fade-in">
                  <CardHeader className="flex flex-row items-center gap-3 p-4 pb-2">
                    ...
                    <div className="overflow-hidden">
                      <CardTitle className="truncate text-sm leading-tight font-bold">
                        {selectedNode.name}
                      </CardTitle>
                      <div className="mt-1 flex flex-wrap gap-1.5">
                        <span className="inline-block rounded-md bg-primary/10 px-2 py-0.5 text-[10px] font-semibold tracking-wide text-primary uppercase">
                          {getRoleBadgeLabel(selectedNode.role)}
                        </span>
                        {selectedNode.level !== undefined && (
                          <span className="inline-block rounded-md bg-indigo-500/10 px-2 py-0.5 text-[10px] font-semibold tracking-wide text-indigo-500 uppercase">
                    ...
                    <div>
                      ...
                      {selectedNodeRelationships.length === 0 ? (
                        <p className="text-xs text-muted-foreground italic">
                          Nenhum relacionamento mapeado para os filtros atuais.
                        </p>
                      ) : (
                        <div className="max-h-[160px] space-y-1.5 overflow-y-auto pr-1">
                          {selectedNodeRelationships.map((rel, idx) => (
                            <div
                              ...
                              className="group flex w-full cursor-pointer items-center justify-between rounded-lg border border-border/20 bg-background/5 p-1.5 text-left transition-all hover:border-primary/25 hover:bg-primary/5"
                            >
                              ...
                              <div className="flex shrink-0 items-center gap-1.5">
                                <span className="rounded bg-zinc-100 px-1.5 py-0.5 text-[9px] font-semibold tracking-wider text-muted-foreground uppercase transition-colors group-hover:bg-primary/10 group-hover:text-primary dark:bg-zinc-800">
                                  {rel.typeName}
                                </span>
                                {/* Disconnect button */}
                                {rel.typeName !== "GC" &&
                                  rel.typeName !== "Líder" && (
                                    <Button
                                      ...
                                      className="h-6 w-6 rounded-md text-destructive opacity-0 transition-all group-hover:opacity-100 hover:bg-destructive/10"
                                    >
                      ...
                      <div className="space-y-2">
                        <div>
                          ...
                          >
                            ...
                            <SelectContent>
                              <SelectItem value="is_disciple">
                                Discipulado por...
                              </SelectItem>
                              <SelectItem value="is_discipler">
                                Discipulador de...
                              </SelectItem>
                              <SelectItem value="fellowship">
                                Companheiro (Comunhão)
                              </SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                        ...
                        <Button
                          ...
                          className="h-8 w-full rounded-lg bg-primary/95 text-[11px] font-semibold hover:bg-primary"
                        >
                ...
                <Card className="flex min-h-[180px] flex-col items-center justify-center border-dashed border-border/50 bg-card/10 p-6 text-center text-muted-foreground shadow-sm backdrop-blur-sm">
                  <Info className="mb-2 h-8 w-8 text-muted-foreground opacity-55" />
                  <h4 className="text-xs font-bold tracking-wider uppercase">
                    Nenhum Discípulo Selecionado
                  </h4>
                  <p className="mt-1 text-[11px]">
                    Clique em um círculo no grafo para ver suas informações
                    detalhadas e seus relacionamentos.
                  </p>
                </Card>
              )}
            </div>
        ...
        <div className="space-y-6">
          <Card className="border-border/50 bg-card/20 shadow-sm backdrop-blur-sm">
            <CardContent className="flex flex-col items-end gap-4 p-4 sm:flex-row">
              <form
                onSubmit={handleCreateSectorTab}
                className="flex w-full flex-1 flex-col items-end gap-4 sm:flex-row"
              >
                ...
                <Button
                  ...
                  className="h-11 shrink-0 rounded-xl bg-primary px-6 font-bold text-primary-foreground hover:bg-primary/90"
                >
          ...
          <Card className="flex h-[450px] flex-col overflow-hidden border-border/50 bg-card/25 shadow-sm backdrop-blur-sm">
            <CardHeader className="p-4 pb-2">
              ...
              <CardDescription className="text-xs">
                Os marcadores representam os Grupos Caseiros e suas cores
                indicam o setor correspondente.
              </CardDescription>
            </CardHeader>
            <CardContent className="relative min-h-[300px] flex-1 p-4 pt-0">
              <ChurchMap
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {sectors.map((sec) => {
              ...
              const availableGroups = homeGroups.filter(
                (g) => g.sector_id !== sec.id
              )

              return (
                <Card
                  key={sec.id}
                  className="flex min-h-[300px] flex-col border-border/50 bg-card/20 shadow-sm backdrop-blur-sm"
                >
                  <CardHeader className="flex shrink-0 flex-row items-center justify-between border-b border-border/40 p-4 pb-2">
                    <div>
                      ...
                      <CardDescription className="text-[10px]">
                        {secGroups.length}{" "}
                        {secGroups.length === 1 ? "grupo" : "grupos"}
                      </CardDescription>
                    </div>
                    <Button
                      variant="ghost"
                  ...
                  <CardContent className="flex flex-1 flex-col justify-between gap-4 overflow-hidden p-4">
                    <div className="max-h-[220px] min-h-[120px] flex-1 space-y-2 overflow-y-auto pr-1">
                      {secGroups.length === 0 ? (
                        <p className="py-8 text-center text-[11px] text-muted-foreground italic">
                          Nenhum grupo neste setor.
                        </p>
                      ) : (
                        secGroups.map((group) => {
                          ...
                          return (
                            <div
                              key={group.id}
                              className="flex items-center justify-between rounded-lg border border-border/30 bg-zinc-50 p-2 dark:bg-zinc-950"
                            >
                              <div className="flex-1 overflow-hidden pr-2">
                                ...
                                <p className="truncate text-[10px] text-muted-foreground">
                                  {group.location_text || "Sem endereço"}
                                </p>
                              </div>
                              <div className="flex shrink-0 items-center gap-1">
                                <Button
                                  ...
                                  className="h-6 w-6 rounded text-muted-foreground hover:bg-primary/10 hover:text-primary"
                                ...
                                <Button
                                  ...
                                  size="icon"
                                  onClick={() =>
                                    handleRemoveGroupFromSector(group.id)
                                  }
                                  className="h-6 w-6 rounded text-muted-foreground hover:bg-destructive/10 hover:text-destructive"
                    ...
                    <div className="flex shrink-0 gap-2 border-t border-border/40 pt-3">
                      <Select
                        value={selectedGroupToAdd[sec.id] || ""}
                        onValueChange={(val) =>
                          setSelectedGroupToAdd((prev) => ({
                            ...prev,
                            [sec.id]: val,
                          }))
                        }
                        ...
                        <SelectContent>
                          {availableGroups.map((g) => {
                            ...
                            const currentSecName =
                              g.sector_id &&
                              sectors.find((s) => s.id === g.sector_id)?.name
                            const label = currentSecName
                              ? `GC - ${leaders} (de: ${currentSecName})`
                              : `GC - ${leaders}`
                            return (
            ...
            {sectors.length === 0 ? (
              ...
              <div className="max-h-48 space-y-1.5 overflow-y-auto pr-1">
                {sectors.map((sec) => (
                  <div
                    key={sec.id}
                    className="flex items-center justify-between rounded-lg border border-border/30 bg-zinc-50 p-2 transition-colors hover:border-border/60 dark:bg-zinc-900"
                  >
      ...
      <Dialog
        open={!!selectedHomeGroupToEdit}
        onOpenChange={(open) => !open && setSelectedHomeGroupToEdit(null)}
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">
              Visualize as informações do Grupo Caseiro ou altere suas
              configurações, líderes e localização.
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleSaveHomeGroup} className="space-y-4 pt-2">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    setEditHomeGroupMeetingDay(Number(val))
                  }
                >
              ...
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    setEditHomeGroupLeader2Id(val === "none" ? null : val)
                  }
                >
                  <SelectTrigger className="h-10 w-full bg-background/50 text-xs">
              ...
              <div className="space-y-1 md:col-span-2">
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    setEditHomeGroupSectorId(val === "none" ? null : val)
                  }
                >
                  <SelectTrigger className="h-10 w-full bg-background/50 text-xs">
              ...
              <div className="space-y-1 md:col-span-2">
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    onChange={(e) =>
                      setEditHomeGroupLocationText(e.target.value)
                    }
                    className="h-10 flex-1 rounded-lg bg-background text-xs"
                    required
                  />
                  <Button
                    type="button"
                    ...
                    disabled={
                      geocodingEdit || !editHomeGroupLocationText.trim()
                    }
                    variant="secondary"
                    size="sm"
            ...
            <div className="mt-4 flex justify-end gap-2 border-t border-border/50 pt-4">
              ...
              <Button
                ...
                size="sm"
                className="h-9 rounded-lg bg-primary px-4 text-xs text-primary-foreground hover:bg-primary/95"
              >
...
export function computeStaticHierarchyLayout(
  ...
  treeRoots.sort(
    (a, b) =>
      getRolePriority(a.nodes[0].role) - getRolePriority(b.nodes[0].role)
  )
      ...
      for (let i = 1; i < children.length; i++) {
        ...
        accumulatedRightContour.forEach((leftRightVal, level) => {
          ...
          if (childLeftVal !== undefined) {
            const requiredShift =
              leftRightVal - childLeftVal + minSiblingSpacing
        ...
        child.rightContour.forEach((rightVal, level) => {
          ...
          if (currentMax !== undefined) {
            accumulatedRightContour.set(
              level,
              Math.max(currentMax, childValDeslocado)
            )
          } else {
        ...
        child.leftContour.forEach((val, absoluteLevel) => {
          ...
          tn.leftContour.set(
            ...
            currentMin !== undefined
              ? Math.min(currentMin, valDeslocado)
              : valDeslocado
          )
        })
        ...
        child.rightContour.forEach((val, absoluteLevel) => {
          ...
          tn.rightContour.set(
            ...
            currentMax !== undefined
              ? Math.max(currentMax, valDeslocado)
              : valDeslocado
          )
        })
      })
    ...
    for (let i = 1; i < treeRoots.length; i++) {
      ...
      accumulatedRight.forEach((leftRightVal, level) => {
        ...
        if (rootLeftVal !== undefined) {
          const requiredShift =
            leftRightVal - rootLeftVal + minSiblingSpacing * 1.5
          shift = Math.max(shift, requiredShift)
        }
      })
  ...
  orphansByLevel.forEach((levelOrphans) => {
...
export function RelationshipGraph({
  ...
  const layoutNodes = useMemo(() => {
    ...
    const computed = computeStaticHierarchyLayout(
      nodes,
      disciplerLinks,
      canvasWidth,
      canvasHeight,
  ...
  const drawCanvas = () => {
    ...
    const scaleFactor =
      visibleNodesCount > 250
        ? 0.4
        : visibleNodesCount > 120
          ? 0.6
          : visibleNodesCount > 60
            ? 0.8
            : 1.0
    ...
    const linksToDraw =
      isolateSelectedConnectionsRef.current && selectedNodeIdRef.current
        ? simLinksRef.current.filter(
            (link) =>
              link.source === selectedNodeIdRef.current ||
              link.target === selectedNodeIdRef.current
          )
        : simLinksRef.current

    const connNodeIds = connectedNodeIdsRef.current
    const nodesToDraw =
      isolateSelectedConnectionsRef.current &&
    ...
    linksToDraw.forEach((link) => {
      ...
      const p1 = isMarriage
        ...
        : getVisualCenter(sourceNode)
      const p2 = isMarriage
        ? { x: targetNode.x, y: targetNode.y }
        : getVisualCenter(targetNode)
      ...
      if (highlightedNodeIdsRef.current !== null) {
        isHighlighted =
          highlightedNodeIdsRef.current.has(link.source) &&
          highlightedNodeIdsRef.current.has(link.target)
      } else if (connectedNodeIdsRef.current !== null) {
        isHighlighted =
          connectedNodeIdsRef.current.has(link.source) &&
          connectedNodeIdsRef.current.has(link.target)
      }
      ...
      const sourceHasSpouse =
        !isMarriage &&
        sourceNode.spouseId &&
        activeNodesMap.has(sourceNode.spouseId)
      const targetHasSpouse =
        !isMarriage &&
        targetNode.spouseId &&
        activeNodesMap.has(targetNode.spouseId)
      ...
      if (hasArrow) {
        ...
        const arrowLength =
          visibleNodesCount > 250 ? 5 : visibleNodesCount > 120 ? 6 : 8
    ...
    nodesToDraw.forEach((node) => {
      ...
      const isHovered =
        hoveredNodeRef.current && hoveredNodeRef.current.id === node.id
      ...
      const spouseNode = node.spouseId
        ? activeNodesMap.get(node.spouseId)
        : null
      const isMarriedToPresbyter = spouseNode?.role === "presbyter"

      if (
        connectionTypeRef.current === "discipler" &&
        node.role !== "presbyter" &&
        node.role !== "child" &&
        !isMarriedToPresbyter &&
        !node.disciplerId
      ) {
      ...
      const isHighlighted =
        ...
        (connectedNodeIdsRef.current &&
          connectedNodeIdsRef.current.has(node.id))
      const showLabel =
        isHighlighted ||
        visibleNodesCount <= 120 ||
        (node.role !== "disciple" &&
          node.role !== "child" &&
          visibleNodesCount <= 220)
    ...
    if (hoveredNodeRef.current) {
      ...
      const levelPrefix =
        connectionTypeRef.current === "discipler" && node.level !== undefined
          ? `Nível ${node.level} • `
          : ""
      const textLine2 = `${levelPrefix}${getRoleLabel(node.role)} • GC: ${node.homeGroupName || "Sem GC"}`

      ctx.font =
        "11px Geist, Outfit, Inter, system-ui, -apple-system, sans-serif"
      ...
      ctx.font =
        "bold 11px Geist, Outfit, Inter, system-ui, -apple-system, sans-serif"
      ctx.fillText(textLine1, tooltipX, ry + 11)

      ctx.fillStyle = isDark ? "#a1a1aa" : "#71717a"
      ctx.font =
        "10px Geist, Outfit, Inter, system-ui, -apple-system, sans-serif"
  ...
  const handleMouseUpOrLeave = () => {
    if (clickedNodeIdRef.current) {
      if (!hasDraggedRef.current) {
        onSelectNode(
          clickedNodeIdRef.current === selectedNodeIdRef.current
            ? null
            : clickedNodeIdRef.current
        )
      }
  ...
  return (
    <div className="relative flex h-[600px] min-h-[500px] flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-md backdrop-blur-sm dark:border-zinc-800 dark:bg-zinc-900/60 dark:shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
      {/* Legenda dos links */}
      <div className="pointer-events-none absolute top-4 left-4 z-10 flex max-w-[80%] flex-wrap gap-2">
        <div className="pointer-events-auto flex items-center gap-4 rounded-xl border border-border/50 bg-background/90 px-3 py-1.5 text-xs shadow-sm backdrop-blur dark:bg-zinc-900/90">
          <span className="font-semibold text-muted-foreground">Linhas:</span>
          <span className="flex items-center gap-1.5">
            <span className="inline-block h-0.5 w-4 bg-indigo-500"></span>{" "}
            Discipulado
          </span>
          <span className="flex items-center gap-1.5">
            <span className="inline-block h-0.5 w-4 border-t border-dashed border-teal-500"></span>{" "}
            Companheiros
          </span>
          <span className="flex items-center gap-1.5">
            <span className="inline-block h-0.5 w-4 border-t border-dotted border-amber-500"></span>{" "}
            Irmãos do GC
          </span>
          <span className="flex items-center gap-1.5">
            <span className="inline-block h-0.5 w-4 bg-rose-500"></span>{" "}
            Casamento
          </span>
        </div>
      </div>
      ...
      {nodes.length === 0 && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-zinc-950/5 p-8 text-center">
          <Network className="mb-2 h-12 w-12 text-zinc-400" />
          <h4 className="text-lg font-semibold text-foreground">
            Nenhum discípulo coincide com os filtros
          </h4>
          <p className="mt-1 text-sm text-muted-foreground">
            Ajuste os filtros de setor, GC ou papéis na barra lateral.
          </p>
        </div>
      )}
